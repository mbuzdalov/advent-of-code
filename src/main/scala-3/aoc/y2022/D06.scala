...
object D06 extends Runner:
  ...
  override def part1(input: IndexedSeq[String]): String = input.map(solve(4)).mkString(" ")
  override def part2(input: IndexedSeq[String]): String = input.map(solve(14)).mkString(" ")
end D06
