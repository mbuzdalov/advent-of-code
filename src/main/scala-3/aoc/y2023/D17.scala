...
object D17 extends Runner:
  ...
  override def part1(input: IndexedSeq[String]): String =
    ...
    while q.nonEmpty do
      ...
      if curr.dist == h(curr.state) then
        ...
        if nr >= 0 && nc >= 0 && nr < nRows && nc < nCols then
          ...
          Loops.foreach(0, 4): d =>
            if d != ((curr.state.dir + 2) & 3) && (d != curr.state.dir || curr.state.same <= 1) then
    ...
    h.filter(p => p._1.row == nRows - 1 && p._1.col == nCols - 1).values.min.toString
    ...
    while q.nonEmpty do
      ...
      if curr.dist == h(curr.state) then
        ...
        if nr >= 0 && nc >= 0 && nr < nRows && nc < nCols then
          val nd = curr.dist + input(nr)(nc) - '0'
          Loops.foreach(0, 4): d =>
            if d != ((curr.state.dir + 2) & 3) && (d != curr.state.dir || curr.state.same <= 8) && (d == curr.state.dir || curr.state.same >= 3) then
              val newState = State(nr, nc, d, if d == curr.state.dir then curr.state.same + 1 else 0)
              if !h.contains(newState) || h(newState) > nd then
                h.put(newState, nd)
                q.addOne(StatePair(newState, nd))

    h.filter(p => p._1.row == nRows - 1 && p._1.col == nCols - 1 && p._1.same >= 4).values.min.toString
end D17
